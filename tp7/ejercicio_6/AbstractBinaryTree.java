...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E>
  ...
  public boolean full() {
    if (isEmpty()) {
      return true; // An empty tree trivially satisfies the condition
    }
    ...
    while (!queue.isEmpty()) {
      ...
      if (numChildren(p) != 2) {
        return false; // Found a node with less than two children
      }
      if (left(p) != null) {
        queue.add(left(p));
      }
    ...
    return true; // All nodes have exactly two children
  }
  ...
  public boolean validExpressionTree(List<E> op) {
    if (isEmpty()) {
      return false; // Un árbol vacío no puede ser un árbol binario de expresiones
    }
    return validateExpressionSubtree(root(), op);
  }

  private boolean validateExpressionSubtree(Position<E> p, List<E> op) {
    boolean isLeaf = (left(p) == null) && (right(p) == null);

    if (isLeaf) {
      return !op.contains(p.getElement()); // Nodo hoja debe ser operando (no operador)
    } else {
      if (op.contains(p.getElement())) { // Nodo interno debe ser operador
        boolean leftValid = (left(p) != null) && validateExpressionSubtree(left(p), op);
        boolean rightValid = (right(p) != null) && validateExpressionSubtree(right(p), op);
        return leftValid && rightValid;
      } else {
        return false; // Nodo interno no es operador
      }
    }
  }
}
