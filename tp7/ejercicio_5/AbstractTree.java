...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  public Iterable<Position<E>> ancestor(Position<E> p) {
    List<Position<E>> ancestors = new LinkedList<>();
  ...
  public Iterable<Position<E>> listGreaterAncestor() {
    List<Position<E>> longestPath = new ArrayList<>();
  ...
  public Iterable<E> listDepth(int depth) {
    List<E> valuesAtDepth = new ArrayList<>();
    for (Position<E> p : positions()) {
      if (depth(p) == depth) {
        valuesAtDepth.add(p.getElement());
      }
    }
    return valuesAtDepth;
