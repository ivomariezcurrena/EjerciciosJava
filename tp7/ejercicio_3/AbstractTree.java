...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  public Iterable<Position<E>> breadthfirst() {
    List<Position<E>> snapshot = new ArrayList<>();
    if (!isEmpty()) {
      Stack<Position<E>> fringe = new Stack<>();
      fringe.push(root()); // empezar con la raíz

      while (!fringe.isEmpty()) {
        Position<E> p = fringe.pop(); // remover del tope de la pila
        snapshot.add(p); // reportar esta posición
        for (Position<E> c : children(p))
          fringe.push(c); // añadir hijos al tope de la pila
      }
    }
    return snapshot;
  }
