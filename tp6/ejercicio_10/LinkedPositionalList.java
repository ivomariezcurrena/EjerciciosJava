...
public class LinkedPositionalList<E> implements PositionalList<E> {
    ...
    /**
     * Retorna el índice corriente del elemento almacenado en la posición p.
     * 
     * @param p la posición cuyo índice se desea obtener
     * @return el índice del elemento almacenado en la posición p
     */
    public int indexOf(Position<E> p) {
        int index = 0; // Inicializar el índice en cero
        Iterator<Position<E>> iterator = positions().iterator();

        // Iterar sobre las posiciones de la lista utilizando un iterador
        while (iterator.hasNext()) {
            Position<E> pos = iterator.next();
            // Comparar la posición actual con la posición dada
            if (pos == p) {
                return index; // Devolver el índice si las posiciones coinciden
            }
            index++; // Incrementar el índice para la siguiente posición
        }

        // Si la posición dada no se encuentra en la lista, devolver -1
        return -1;
    }
